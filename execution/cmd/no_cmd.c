...
/*   Updated: 2024/05/30 15:33:33 by hboudar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../execution.h"

static int  redirectout_nocmd(t_prompt *prompt, int *fd, int *fd1)
{
...
static int	redirectin_nocmd(t_prompt *prompt, int *fd, int *fd0)
{
...
int    no_cmd(t_prompt *prompt)
{
    ...
    {
        if (!prompt->cmd->file->type && !redirectin_nocmd(prompt, fd, &fd0))
            return (1);
        else if (prompt->cmd->file->type && !redirectout_nocmd(prompt, fd, &fd1))
            return (1);
